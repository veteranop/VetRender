# FIXED calculate_propagation function
...
def calculate_propagation(self):
    ...
    try:
        ...
        if self.use_terrain.get():
            ...
            # Convert Cartesian grid to polar, interpolate, then apply back
            ...
            interp = RectBivariateSpline(sample_distances, sample_azimuths_wrap,
                                         terrain_loss_wrap, kx=3, ky=3)
            
            # Interpolate for each Cartesian grid point
            for i in range(dist_grid.shape[0]):
                for j in range(dist_grid.shape[1]):
                    if not mask[i, j] and dist_grid[i, j] > 0.1:
                        terrain_loss_grid[i, j] = float(interp(dist_grid[i, j], az_grid[i, j]))
